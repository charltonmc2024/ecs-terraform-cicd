...
import { FiMail, FiUser, FiMessageSquare, FiPhone, FiSend, FiX, FiCheckCircle, FiAlertCircle } from "react-icons/fi";
...
const ContactSupport = () => {
  ...
  const [selectedPlans, setSelectedPlans] = useState<PlanOption[]>([]);

  const handlePlanChange = (option: PlanOption, checked: boolean) => {
    setSelectedPlans((prev) => {
      if (checked) {
        return prev.includes(option) ? prev : [...prev, option];
      }
      return prev.filter((plan) => plan !== option);
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!firstName || !lastName || !email || !message) {
      setToast({ type: "error", message: "Please complete all required fields before sending." });
      setTimeout(() => setToast(null), 5000);
      return;
    }

    if (selectedPlans.length === 0) {
      setToast({ type: "error", message: "Please select at least one product option." });
      setTimeout(() => setToast(null), 5000);
      return;
    }
    ...
    try {
      const res = await fetch("/api/support", {
        ...
        body: JSON.stringify({
          ...
          email,
          message,
          productsOfInterest: selectedPlans,
          formType: "contact",
        }),
      });
  ...
  return (
    <section id="contact" className="relative overflow-hidden pt-[120px] pb-16 md:pb-20 lg:pb-28">
        ...
        <div className="mx-auto max-w-5xl">
          ...
          >
            <div
              ...
              style={{
                background: `
                  radial-gradient(120% 80% at 50% -10%, rgba(14,165,233,0.22) 0%, transparent 55%),
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="-mx-4 flex flex-wrap">
                    ...
                    <div className="w-full px-4 md:w-1/2">
                      <InputField
                        delay={0.18}
                        id="contactNo" placeholder="Contact No. (Optional)"
                        icon={FiPhone} name="contactNo"
                      />
                    </div>
                    ...
                    <div className="w-full px-4 md:w-1/2 flex">
                      <motion.fieldset
                        className="mb-7 w-full flex flex-col"
                        initial={{ opacity: 0, y: 12 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.5, delay: 0.3, ease: [0.2, 0.8, 0.2, 1] }}
                        aria-label="The Products"
                      >
                        <legend className="sr-only">The Products</legend>
                        <div
                          className="rounded-2xl px-4 py-4 sm:px-5 flex-1 flex flex-col justify-between min-h-[240px]"
                          style={{
                            background: `
                              radial-gradient(120% 150% at 50% -10%, rgba(255,255,255,0.06) 0%, transparent 60%),
                              linear-gradient(180deg, rgba(15,122,163,0.55) 0%, rgba(10,104,140,0.65) 100%)
                        ...
                        >
                          <div className="grid grid-cols-1 gap-y-3">
                            {PLAN_OPTIONS.map((option) => {
                              const inputId = `plan-${option.toLowerCase().replace(/\s+/g, "-")}`;
                              const isChecked = selectedPlans.includes(option);

                              return (
                                <label
                                  key={option}
                                  htmlFor={inputId}
                                  className="flex items-center gap-3 cursor-pointer rounded-xl px-2 py-1.5 transition-colors duration-200 hover:bg-white/5"
                                >
                                  <input
                                    id={inputId}
                                    type="checkbox"
                                    ...
                                    value={option}
                                    checked={isChecked}
                                    onChange={(e) => handlePlanChange(option, e.target.checked)}
                                    className="h-4 w-4 shrink-0 rounded border-white/40 bg-transparent text-[#0A688C] accent-[#7dd3fc] focus:ring-2 focus:ring-sky-300/40 focus:ring-offset-0"
                                  />
                                  <span className="text-[15px] text-white font-medium leading-snug">
                                    {option}
                                  </span>
                                </label>
                              );
                          ...
                          <p className="mt-4 text-xs text-white/70">
                            Select one or more plans. At least one option is required.
                          </p>
                        </div>
                      </motion.fieldset>
                    </div>
                    ...
                    <div className="w-full px-4 md:w-1/2">
                      <InputField
                        ...
                        icon={FiMessageSquare} name="message" required rows={9}
                      />
                    </div>
